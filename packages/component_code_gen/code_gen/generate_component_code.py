...
def generate_code(app, prompt, templates, parsed_common_files, tries):
    ...
    for i in range(tries):
        ...
        if 'docs_url' in docs_meta:
            ...
            if contents:
                ...
                results.append(call_langchain(
                    app, prompt, templates, auth_example, parsed_common_files, docs, 'api reference'))
        ...
        if 'openapi_url' in docs_meta:
            ...
            if contents:
                ...
                results.append(call_langchain(
                    app, prompt, templates, auth_example, parsed_common_files, docs, 'openapi'))
                has_docs_result = True

        # If we haven't obtained any results using docs
        if not has_docs_result:
            results.append(call_langchain(app, prompt, templates, auth_example, parsed_common_files))
...
def call_langchain(app, prompt, templates, auth_example, parsed_common_files="", docs=None, docs_type=None, attempts=0, max_attempts=3):
    ...
    if not docs:
        ...
        return langchain_helpers.no_docs(app, prompt, templates, auth_example, parsed_common_files)
    ...
    result = langchain_helpers.ask_agent(prompt, docs, templates, auth_example, parsed_common_files)

    if result != "I don't know":
        return result

    logger.debug("Trying again without docs")
    return call_langchain(app, prompt, templates, auth_example, parsed_common_files, attempts=attempts+1)
